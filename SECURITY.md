...
added at Session 5 when the project has a public contact channel).
...
This policy covers the `privacy-forge` application code, its Docker images,
and its GitHub Actions workflows. It does not cover third-party dependencies
(report those upstream) or self-hosted deployments the maintainer does not
operate.
...
- Threat model: `docs/project-memory/06-security-threat-model.md` (produced
  at Session 4)
- OWASP ASVS L2 mapping: `docs/security/asvs-mapping.md` (produced at
  Session 4)
