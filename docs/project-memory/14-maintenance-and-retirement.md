...
| Laravel | `^12.61.1` (`composer.json`) — **note, corrected at Session 19, root cause traced at Session 20**: `00a-ledger-confirmation.md`'s frozen Session-0 ledger row names "Laravel 11" as the originally-allocated framework; the codebase has actually run on Laravel 12 since early in Session 5 (an undocumented drift, traced to commit `97868f1` and formally decided via **ADR-0008** at Session 20 — not "since Session 13," which was merely the earliest session this document previously cited before the real mechanism was found). This document cites what is actually installed, not the stale ledger figure — the ledger itself is explicitly "not modified again" per its own governance note, so this correction lives here instead. | composer.json | Laravel 11: EOL 2026-03-12 (**already past** as of this session — moot for this repo since it's not what's installed). Laravel 12: active support ends 2026-08-16 (also already passed as of this session's date, 2026-08-17 — worth a fast follow-up look, see note below); **security-only support until 2027-02-24**. |
