...
> Last updated: 2026-09-19 (Session 29 — T-03's rate limiting implemented
> for real; T-16's DB-grant claim found fabricated and corrected (see
> `10-risk-register.md`'s R-09); T-01/T-04/T-05's "Verified by" citations
> audited against the actual test suite and corrected where they named
> nonexistent files)
...
| T-16 | B4 PolicyEvaluator (internal) | Policy definitions edited directly at the database level, bypassing the application (e.g. a compromised DB credential) | Tampering | Low/**Critical** | **Correction, this session — the DB-level grant restriction this row describes does not exist in the codebase.** No migration issues a `GRANT`/`REVOKE` against `policy_definitions` (contrast with `audit_log_entries`, whose grant restriction is real — see `AuditLogGrantEnforcementTest.php` and the Session 7 migration that revokes `UPDATE`/`DELETE` from the app runtime role). The application-level control still holds: legitimate policy changes go through the `policy.update` sensitive action (Owner-only, audit-logged, gated by the fail-closed evaluator), but the DB-level defence-in-depth layer this row claimed was never built. Logged as R-09 in `10-risk-register.md`; out of scope for this session (rate-limiting work only) to implement | Previously cited `tests/Feature/PolicyDefinitionGrantTest.php`, which does not exist and never has — removed this session, not corrected to a real filename, because no equivalent test exists either |
