...
> Last updated: 2026-09-19 (Session 30 — T-16's DB-grant gap, found
> fabricated at Session 29 (R-09), closed for real with a column-scoped
> Postgres grant restriction (ADR-0009); Session 29 — T-03's rate limiting
> implemented for real; T-01/T-04/T-05's "Verified by" citations audited
> against the actual test suite and corrected where they named nonexistent
> files)
...
| T-03 | B1 Public consent API | Volumetric abuse of the capture/withdraw endpoints | Denial of Service | Med/Low | **Implemented this session:** IP-level rate limiting on both `POST /consent` and `POST /consent/{consentId}/withdraw` (distinct from the per-subject DSAR limit in NFR-006), keyed on caller IP via `RateLimiter`, returning `429` with a `ProblemDetail` body past the configured threshold — see `config/consent.php` (`CONSENT_CAPTURE_RATE_LIMIT_PER_MINUTE`/`CONSENT_WITHDRAW_RATE_LIMIT_PER_MINUTE`, default 30/min each). Previously this row cited a load test that does not exist in this codebase and no rate-limiting code existed at all — that gap is what this session closes | `tests/Feature/ConsentRateLimitTest.php` (this session) |
| T-04 | B1 Public DSAR portal | Attacker submits a DSAR impersonating a real data subject to gain unauthorised access to, or erase, that subject's data | Spoofing | High/**Critical** | Manual identity-verification gate (FR-007) is mandatory before any export/erasure task executes — deliberately conservative (a human check) precisely because this is the highest-impact spoofing surface in the system | `tests/Feature/DsarIdentityVerificationTest.php` + `tests/Feature/DsarErasureApprovalTest.php` (Session 7; filename corrected this session — previously cited the nonexistent `DsarIdentityGateTest.php`) |
| T-05 | B1 Public DSAR portal | DSAR/status identifiers are enumerable, leaking the existence or status of other subjects' requests | Information Disclosure | Med/Med | Status and export access are keyed only by unguessable signed tokens (not sequential/plain DSAR IDs); no endpoint accepts a bare DSAR ID from an unauthenticated caller | `tests/Feature/DsarStatusTest.php` (Session 7; filename corrected this session — previously cited the nonexistent `DsarTokenOnlyAccessTest.php`) |
...
| T-16 | B4 PolicyEvaluator (internal) | Policy definitions edited directly at the database level, bypassing the application (e.g. a compromised DB credential) | Tampering | Low/**High** (reduced from Critical — see below) | **Real fix, this session (ADR-0009), closing R-09.** Unlike `audit_log_entries` (R-01), `policy_definitions` is not append-only — `PolicyController::update()` legitimately issues a real `UPDATE ... SET status = ...` as part of its own versioning workflow, so a blanket `UPDATE`/`DELETE` revoke copied from R-01 would break that feature. Instead: `DELETE` is revoked entirely from the app runtime role (the application never deletes a policy row), and `UPDATE` is narrowed with Postgres column-level grants to exactly the two columns the versioning workflow writes (`status`, `updated_at`) — see `2026_09_19_000002_restrict_runtime_role_grant_for_policy_definitions.php`. A raw SQL `UPDATE` of `effect`/`action_name`/any condition column, or any `DELETE`, is now rejected by Postgres itself (`42501`, insufficient_privilege) for the exact runtime credential the application uses, even though that same credential still legitimately has `SELECT`/`INSERT`/`UPDATE(status)`. **Severity reduced from Critical to High, not to Low:** this is real, verified defence-in-depth against the specific tampering shape T-16 names most directly (silently rewriting an existing, already-trusted row's substantive fields), but it does not make the table fully tamper-evident the way `audit_log_entries` is — a compromised runtime credential can still flip `status` directly or insert a new "version" row bypassing the ABAC gate and audit log entirely; see ADR-0009's own "Trade-offs accepted" for the honest boundary of what this control does and does not cover | `tests/Feature/PolicyDefinitionGrantEnforcementTest.php` (this session) — connects as the real app runtime role and issues raw SQL against `policy_definitions`, the same standard `AuditLogGrantEnforcementTest.php` (R-01) holds itself to. The underlying Postgres grant/revoke mechanism was additionally verified directly against a real local Postgres 16 instance this session (reproducing the pre-fix gap — a raw `UPDATE`/`DELETE` as the runtime role succeeded — then confirming the post-fix migration SQL rejects both with `42501` while `UPDATE(status)`/`INSERT`/`SELECT` keep working); the Pest test file itself could not be executed in this session's sandbox (`composer install` cannot complete here — this session's GitHub access is scoped to this one repository only, so every other Composer/Packagist dependency's GitHub-hosted source is unreachable — a distinct, narrower cause than the transient rate-limiting a prior session hit for the same symptom), so its first real run is the PR's CI, not a local Pest run |
