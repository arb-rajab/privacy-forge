...
| `DELETION_CERTIFICATE` | Evidence of what was (or wasn't) erased | summary, exceptions | Evidentiary record. Implemented at Session 8 (`App\Services\DeletionCertificateGenerator`). Generated once every erasure task reaches a terminal state, success or not — `exceptions` stays `null` only when every connector confirmed; otherwise it names which connector(s) could not confirm and why (FR-011). `retention_execution_id` remains nullable and unpopulated: the retention-execution path (US-012) that would also produce certificates doesn't exist yet. |
