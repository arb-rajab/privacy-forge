...
| 5. Verification & Testing | baseline | CI runs 187 Pest tests automatically on every PR (consent, DSAR, ABAC/`PolicyEvaluator` fail-closed fault injection, retention parity, RoPA, audit-chain tamper simulation, connector dispatch/callback), including an exhaustive (role × action) authorisation matrix (NFR-005). One accepted residual gap: the browser-driven E2E suite (`tests/Browser/`) is not part of this automated count — see `10-risk-register.md`'s R-08 | `tests/`, `.github/workflows/ci.yml`, `docs/project-memory/10-risk-register.md` |
| 6. Release & Deployment | baseline | CI pipeline: lint, static analysis, tests, gitleaks, CodeQL, osv-scanner, OpenAPI validation, and a `dependency-governance` job (ADR-0008) — all running automatically on every PR. Multi-stage Dockerfiles for dev and production; a real production stack (`docker-compose.prod.yml`) built and verified locally over genuinely validated HTTPS against a placeholder domain (Sessions 22-24). Real, paid-for public infrastructure is explicitly descoped for this portfolio build — see `09-decision-log.md`'s Session 24 entry | `.github/workflows/ci.yml`, `docker/Dockerfile`, `docker-compose.prod.yml`, `docs/project-memory/08-deployment-and-operations.md` |
| 7. Operations & Maintenance | light | Baseline runbooks and dashboards/alerts exist (demo-viewer credential rotation, audit-chain anchor-failure alerting per ADR-0003) — intentionally kept light rather than deep, since full operational depth (SLO-driven alerting, incident response) is demonstrated elsewhere in the portfolio, in `pulsewatch` (R03) | `docs/project-memory/08-deployment-and-operations.md` |
...
See also [`docs/CASE-STUDY.md`](CASE-STUDY.md) for a narrative synthesis
across sessions — real bugs found and fixed, the Laravel-version
governance incident (ADR-0008), and the honest judgment calls behind R-08
and the demo-hosting decision — and [`docs/demo/`](demo/) for real
